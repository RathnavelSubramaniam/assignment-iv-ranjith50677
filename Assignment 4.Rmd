...
```
  explain:- The estimated standard errors for the coefficients associated with income and balance is 4.985e-06 and             2.274e-04.
...
```
  ...
  explain :- The boot.fn function return the coefficient value of the income 2.080898e-05 and balance                    5.647103e-03.
...
```
  ...
  explain:- Finding the Glm function for the variable Lag1 and Lag2
...
```
  ...
  explain:- The First Column Value is greater than 0.5 and the result is true and the direction is Down so the first value is wrong
...
```
  ...
  explain:- Here we use for loop and check thee value for all values column is true and the error row value has changed to 0 and 1 by refering it as true or false.
